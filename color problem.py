...
player= pygame.Rect(position_playerX + x, position_playerY + y, size_player, size_player)
...
class Camera:
    def __init__(self, largura, altura):
        self.cam = pygame.Rect(0, 0, largura, altura)
        self.largra = largura
        self.altura = altura
    def apply(self, rect):
        return rect.move(self.cam.topleft)
    def update_cam(self, player):
        x = -position_playerX + largura // 2
        y = -position_playerY + altura // 2

        x = min(0, x)
        y = min(0, y)
        x = max(-(largura- largura), x)
        y = max (-(altura - altura), y)

        self.cam = pygame.Rect(x, y, largura, altura)
